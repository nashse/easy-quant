 ...
 */
@Slf4j
public class TripleFilterTradingStrategybackup extends BaseStrategy {
...
//            log.info("rsi指标值num : " + num);
    ...
    private void thirdFilterScreen() {
        ...
        List<Kline> klines = mdE.getKline(symbol, granularityMin);
        Collections.reverse(klines);
        ...
        if (bLongPosition()) {
            ...
            if (!bTriggerOrder()) {
                ...
                BigDecimal currLowPrice = klines.get(klines.size() - 1).getLow();
                Double stopPrice = Math.min(preLowPrice.doubleValue(), currLowPrice.doubleValue());
                ...
                log.info("currLowPrice : " + currLowPrice);
                id = CommomUtil.repeatOrderTrigger(tradeE, "triggerCloseLong", "-1", 5,
                        symbol, lossPrice, quantity, -1);
            }
            ...
            if (!bTriggerOrder()) {
                ...
                BigDecimal currHighPrice = klines.get(klines.size() - 1).getHigh();
                Double stopPrice = Math.max(preHighPrice.doubleValue(), currHighPrice.doubleValue());
                ...
                log.info("currHighPrice : " + currHighPrice);
                id = CommomUtil.repeatOrderTrigger(tradeE, "triggerCloseShort", "-1", 5,
                        symbol, lossPrice, quantity, -1);
            }
        ...
        BigDecimal thirdHighPrice = klines.get(klines.size() - 3).getHigh();
        BigDecimal twoHighPrice = klines.get(klines.size() - 2).getHigh();
        if (SingleState.LONG.equals(singleStateMid) &&
                thirdHighPrice.compareTo(twoHighPrice) == 1) {
            cancelPendingOrdersAll();
            cancelTriggerPendingAll();
            log.info("thirdHighPrice : " + thirdHighPrice);
            log.info("twoHighPrice : " + twoHighPrice);
            id = CommomUtil.repeatOrderLimit(tradeE, "openLong", "-1", 5,
                    symbol, thirdHighPrice, quantity, -1);
        }

        BigDecimal thirdLowPrice = klines.get(klines.size() - 3).getLow();
        BigDecimal twoLowPrice = klines.get(klines.size() - 2).getLow();
        if (SingleState.SHORT.equals(singleStateMid) &&
                thirdLowPrice.compareTo(twoLowPrice) == -1) {
            cancelPendingOrdersAll();
            cancelTriggerPendingAll();
            log.info("thirdLowPrice : " + thirdLowPrice);
            log.info("twoLowPrice : " + twoLowPrice);
            id = CommomUtil.repeatOrderLimit(tradeE, "openShort", "-1", 5,
                    symbol, thirdLowPrice, quantity, -1);
        }
    }
